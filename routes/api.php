        ...
        Route::middleware(['auth:api', 'api.admin'])->group(function () {
            ...
            Route::middleware('api.permission:access production')->prefix('production')->group(function () {
                ...
                Route::prefix('jobs')->group(function () {
                    ...
                    Route::get('/debug-35', function() {
                        $job = \App\Models\ProductionJob::find(35);
                        $costingService = app(\App\Services\Manufacturing\ProductionCostingService::class);
                        $batch = $job->batch;
                        $batchConsumptions = \App\Models\ProductionBatchConsumption::where('production_batch_id', $batch->id)->get();
                        $jobConsumptions = \App\Models\JobMaterialConsumption::where('production_job_id', 35)->get();
                        $allBatchJobConsumptions = \App\Models\JobMaterialConsumption::whereIn('production_job_id', $batch->jobs->pluck('id'))->get();
                        $jobCostSummary = $costingService->getJobCostSummary(35);
                ...
                // Raw Material Categories Management
                Route::prefix('raw-material-categories')->group(function () {
                    Route::get('/', [\App\Http\Controllers\Api\V1\Admin\AdminRawMaterialCategoryController::class, 'index']);
                    Route::get('/{id}', [\App\Http\Controllers\Api\V1\Admin\AdminRawMaterialCategoryController::class, 'show'])->where('id', '[0-9]+');
                    Route::post('/', [\App\Http\Controllers\Api\V1\Admin\AdminRawMaterialCategoryController::class, 'store']);
                ...
                // Fabric Width Master Management
                Route::prefix('fabric-widths')->group(function () {
                    Route::get('/', [\App\Http\Controllers\Api\V1\Admin\AdminFabricWidthController::class, 'index']);
                    Route::get('/options', [\App\Http\Controllers\Api\V1\Admin\AdminFabricWidthController::class, 'options']);
                    Route::get('/{id}', [\App\Http\Controllers\Api\V1\Admin\AdminFabricWidthController::class, 'show'])->where('id', '[0-9]+');
