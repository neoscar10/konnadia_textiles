    ...
    public function toArray(Request $request): array
    {
        return [
            ...
            'dispatch_summary' => [
                'status' => $this->status,
                'status_label' => $this->status_label,
                'total_items_count' => (int) $this->items->count(),
                'dispatched_items_count' => (int) $this->items->where('status', 'dispatched')->count(),
                'pending_items_count' => (int) $this->items->where('status', '!=', 'dispatched')->where('status', '!=', 'cancelled')->count(),
                'cancelled_items_count' => (int) $this->items->where('status', 'cancelled')->count(),
                'is_partially_dispatched' => in_array($this->status, ['partially_dispatched', 'partially_dispatched_balance_cancelled']),
                'is_fully_dispatched' => $this->status === 'dispatched',
            ],
