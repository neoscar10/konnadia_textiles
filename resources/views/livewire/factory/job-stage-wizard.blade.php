                    ...
                    <div class="bg-white rounded-2xl border border-gray-200/80 p-6 shadow-sm space-y-6">
                        ...
                        <div class="pt-4 border-t border-slate-200 flex items-center justify-between">
                            ...
                            @else
                                ...
                                <button type="button" wire:click="completeActiveStage" class="px-8 py-3.5 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs rounded-xl transition-all shadow-md active:scale-95 flex items-center gap-2 cursor-pointer">
                                    <span class="material-symbols-outlined text-[18px]">check_circle</span>
                                    <span>{{ $isFinalTask ? 'Confirm & Complete Final Task' : 'Confirm & Complete Stage' }}</span>
                                </button>
                            @endif
                        </div>
                    </div>
                        ...
                        <div class="space-y-3 max-h-80 overflow-y-auto pr-1">
                            ...
                            <div class="space-y-3">
                                @foreach($baleRollLengths as $i => $len)
                                    ...
                                    @endphp
                                    <div wire:key="bale-roll-card-{{ $i }}-{{ $selectedRollMatId }}" class="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-2">
                                        <div class="flex items-center justify-between">
                                            ...
                                            @if(!empty($baleRollLengths[$i]) && (float)$baleRollLengths[$i] > 0 && strtoupper($unitShortCode) !== 'M')
                                                <span class="px-2 py-0.5 bg-amber-100 text-amber-900 font-mono text-[10px] font-black rounded border border-amber-300">
                                        ...
                                        <div class="grid grid-cols-1 sm:grid-cols-12 gap-2.5">
                                            ...
                                            <div class="sm:col-span-3">
                                                ...
                                                <select wire:model.live="baleRollMaterials.{{ $i }}" wire:key="roll-mat-select-{{ $i }}" class="w-full px-2.5 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-bold text-slate-900">
                                            ...
                                            <div class="sm:col-span-3">
                                                ...
                                                @endphp
                                                <select wire:model.live="baleRollWidths.{{ $i }}" wire:key="roll-width-select-{{ $i }}-{{ $selectedRollMatId }}-{{ $baleRollWidths[$i] ?? '' }}" class="w-full px-2.5 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-bold text-slate-900">
                                            ...
                                            <!-- Measured Length & Dynamic Unit -->
                                            <div class="sm:col-span-3">
                                                <label class="block text-[10px] font-bold text-slate-500 mb-0.5">Roll Length &amp; Unit *</label>
                                                <div class="flex items-center gap-1">
                                                    <input type="number" step="0.01" wire:model.live.debounce.300ms="baleRollLengths.{{ $i }}" placeholder="0.00" class="w-full px-2 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-bold text-slate-900">
                                                    @php
                                                        $availUnits = $this->getAvailableUnitsForMaterial($selectedRollMatId);
                                                    @endphp
                                                    <select wire:model.live="baleRollUnits.{{ $i }}" wire:key="roll-unit-select-{{ $i }}-{{ $selectedRollMatId }}" class="px-1.5 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-extrabold text-amber-800 shrink-0">
