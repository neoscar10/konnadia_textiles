<nav class="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-outline-variant/30 px-2 py-1 pb-safe z-50 flex items-center justify-around shadow-lg">
    <!-- Home Tab -->
    <a href="{{ route('home') }}" wire:navigate class="flex flex-col items-center gap-0.5 py-1 px-3 rounded-xl transition-all {{ request()->routeIs('home') || request()->routeIs('customer.dashboard') ? 'text-gold' : 'text-slate-500' }}">
        <span class="material-symbols-outlined text-2xl" style="font-variation-settings: 'FILL' {{ request()->routeIs('home') || request()->routeIs('customer.dashboard') ? 1 : 0 }}">home</span>
